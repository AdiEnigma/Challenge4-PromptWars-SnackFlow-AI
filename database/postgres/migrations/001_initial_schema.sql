...
-- Creates the full initial PostgreSQL schema for SnackFlow AI structured data.
-- Idempotent: safe to re-run (uses IF NOT EXISTS / CREATE OR REPLACE).
-- NOTE: Applied via pg's pool.query (no psql \i support). Schema is inlined.
...
-- Record this migration (ignored if the runner self-records).
